...
- 공개 페이지와 embed 응답에서 노출되는 이미지/영상 후보를 먼저 보여주고, 영상이 빠진 Reel은 보조 공개 메타 응답에서 MP4 후보를 한 번 더 확인합니다.
...
비공개 게시물, 로그인이 필요한 게시물, 만료되었거나 지역 제한이 있는 미디어는 우회하지 않습니다. 공개 응답과 보조 공개 메타 응답에 다운로드 가능한 미디어 URL이 없으면 앱은 실패 메시지를 보여줍니다.
